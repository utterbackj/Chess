...
public class ChessClient extends Application {
	...
	public void start(Stage primaryStage) {
		...
		moveMaker[Chess.BLACK] = new AIMoveMaker(new ModestAI());
	...
	private class GameHandler implements EventHandler<ActionEvent> {
		...
		public void handle(ActionEvent e) {
			...
			MoveMaker mover0 = moveMaker[0];
			MoveMaker mover1 = moveMaker[1];

			if (mover0.getState() == Worker.State.READY && mover1.getState() == Worker.State.READY) {
				mover.start(game.position);
			} else if (((mover0.getState() == Worker.State.SUCCEEDED && mover1.getState() != Worker.State.RUNNING) ||
					(mover1.getState() == Worker.State.SUCCEEDED && mover0.getState() != Worker.State.RUNNING))
					&& boardView.ready()) {
